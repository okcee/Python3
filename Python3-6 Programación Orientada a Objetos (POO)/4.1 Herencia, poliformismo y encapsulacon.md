...
- Atributos Públicos y Protegidos: Por defecto, los atributos definidos en la clase padre que no tienen un prefijo especial (como _ para protegido o __ para privado) son considerados públicos. Estos atributos siempre se heredan a la subclase. Los objetos de la subclase podrán acceder y modificar estos atributos (si no se han sobrescrito en la subclase). Los atributos protegidos (convencionalmente precedidos por un guion bajo _) también se heredan, aunque la convención sugiere que no deberían ser accedidos directamente desde fuera de la clase o sus subclases.  

- Atributos Privados: Los atributos que tienen un doble guion bajo como prefijo (__) se consideran privados en Python (name mangling). Estos atributos no se heredan directamente a la subclase de la misma manera que los públicos o protegidos. La subclase no podrá acceder a ellos directamente utilizando el mismo nombre. Sin embargo, es importante entender que Python no tiene un sistema de privacidad absoluto. Aún se puede acceder a los atributos privados de la superclase desde la subclase utilizando una forma de "name mangling" ( _NombreClasePadre__nombre_atributo_privado), pero esto generalmente se considera una mala práctica y va en contra del principio de encapsulamiento.  
