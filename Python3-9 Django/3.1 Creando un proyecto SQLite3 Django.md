...
1.  **Configurar la conexión a la base de datos en `settings.py`:** Abre el archivo `miProyecto/settings.py` y busca la sección `DATABASES`. Por defecto, estará configurada para SQLite. Debes modificarla para que apunte a tu instalación de PostgreSQL. La configuración típica sería similar a esta:
...
2.  **Aplicar las migraciones:** Una vez que hayas configurado la conexión a PostgreSQL en `settings.py`, debes ejecutar el comando de migraciones para crear las tablas necesarias en tu base de datos PostgreSQL:
