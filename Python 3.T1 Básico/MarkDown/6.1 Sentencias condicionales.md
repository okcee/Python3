...
for variable in secuencia:  
  ...
  * La **variable**, por convención, i es una variable temporal que se utiliza para almacenar cada carácter de la cadena frase a medida que el bucle for la recorre. Es importante comprender que la variable "i" solo existe dentro del bucle for.
    * Convención general para nombrar la variable:
      * Iterando sobre listas: Usando el contenido de las listas; numero, elemento, palabra, objeto (o nombre del objeto).
      * Iterando sobre cadenas: Dado que una cadena es una secuencia de caracteres, es común usar caracter o letra.
      * Iterando con range(): Con la función range() para generar una secuencia de números, es muy común usar i, j o k como nombres de variables.
      * Iterando con enumerate(): Cuando se utiliza enumerate(), es común usar indice y valor como nombres de variables.
  * **secuencia**: Es una colección de elementos sobre la cual se itera. Puede ser una lista, tupla, cadena, rango u otro objeto iterable.
  * **Bloque de código**: Es el conjunto de instrucciones que se ejecutan para cada elemento de la secuencia.
